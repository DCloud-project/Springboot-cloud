...
public interface UserMapper extends BaseMapper<User> {
    ...
    @Select("SELECT name,sex,email,role_id,state,school_id FROM user WHERE is_delete = 0 AND role_id != 2 AND state = #{state} LIMIT #{offset},10")
    List<Map>selUserByState(int state,int offset);  //根据状态查询用户

    @Select("SELECT name,sex,email,role_id,state,school_id FROM user WHERE is_delete = 0 AND role_id != 2 AND name LIKE #{text} LIMIT #{offset},10")
    List<Map>selUserByName(@Param("text")String text,int offset);  //根据用户名查询用户

    @Select("SELECT name,sex,email,role_id,state,school_id FROM user WHERE is_delete = 0 AND role_id != 2 AND name LIKE #{text} AND state = #{state} LIMIT #{offset},10")
